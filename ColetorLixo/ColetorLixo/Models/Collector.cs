    ...
    public class Collector : Agent
    {
        ...
        public Cell MoveToObjectiveWithAStarAlg(Cell objective, List<Cell> neighbors, List<Cell> visited, Cell actual, MatrixViewModel matrix)
        {
            ...
            foreach (Cell neighbor in neighbors)
            {
                ...
                {
                    double value = CalculateDistance(actual, neighbor);
                    //List<Cell> Nvisitados = new List<Cell>();
                    //visited.CopyTo(Nvisitados);
                    //visited.Add(actual);
                    List<Cell> Nneighbors = GetNeighbors(neighbor, matrix, this.InvalidCells, this.VisitedCells);
                    double value2 = 1000;
                    Cell n2 = null;
